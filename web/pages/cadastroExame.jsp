    ...
    <sql:setDataSource var="ds"
        ...
        <form name="cadastroExame" method="post" action="?action=cadastroExame">
            ...
            <sql:query var="qryExames" dataSource="${ds}">
                ...
                </select>  Preencha este campo somente se necessitar ALTERAR um exame já cadastrado!<br>
            ...
            <sql:query var="qryTipoExames" dataSource="${ds}">
                select * from tipoExame order by nomeExame
            </sql:query>
            <label>Tipo de Exame: </label>
            <select name="codigoTipo">
                <c:forEach var="tipoExames" items="${qryTipoExames.rows}">
                    <option value="${tipoExames.codigoTipo}">${tipoExames.nomeExame}</option>
                </c:forEach>
            </select><br>
            ...
            <label>Codigo/CPF do Cliente: </label><select name="codigoCliente">
                <c:forEach var="clientes" items="${qryClientes.rows}">
                    <option value="${clientes.codigo}">cpf: ${clientes.cpf} -  Cod. ${clientes.codigo}</option>
                </c:forEach>
            </select><br>
            <label>Data de realização do Exame: </label>
            <input type="text" name="dataExame" value="" size="10" /><br><br>
